...
int main()
{
    ...
    char cstr[] = "Hello World! from child";
    char pstr[] = "Hello World! from parent";
    ...
    // parents process
        ...
        if((fd1 = open(fifoPath1,O_WRONLY)) < 0) {
            fprintf(stderr,"Fail to open %s : %s.\n",fifoPath1,strerror(errno));
            exit(EXIT_FAILURE);
        }
        if((fd2 = open(fifoPath2,O_RDONLY)) < 0) {
            fprintf(stderr,"Fail to open %s : %s.\n",fifoPath2,strerror(errno));
            exit(EXIT_FAILURE);
        }
        if(write(fd1, pstr, sizeof(pstr)) < 0) {
            fprintf(stderr,"Fail to write %s : %s.\n",fifoPath1,strerror(errno));
            exit(0);
        }
        printf("I am parent: ");
        if(read(fd2, readBuff, sizeof(readBuff)) > 0)
            printf("%s\n", readBuff);
        close(fd1);
        close(fd2);
        unlink(fifoPath1);
