...
int main(int argc, char** argv) {
    ...
    // 初始化描述符集合
    FD_ZERO(&readfds);   
    FD_SET(con_sockfd, &readfds);//将客户端socket加入到集合中 
    testfds = readfds;
    ...
    // read
    if((msglen = read(con_sockfd, recvBuff, 10)) <= 0) {
        printf("read socket error: %s(errno: %d, return: %d)\n", strerror(errno), errno, msglen);
    }

    // revc
    // if((msglen = recv(con_sockfd, recvBuff, 10, 0)) <= 0) {
    ...
    // for(;;){
    // // for(i = 0; i < 10; i++){
    //     if((msglen = read(con_sockfd, recvBuff, rNum)) <= 0) {
    ...
    //     }
    //     if (write(con_sockfd, sendBuff, wNum) <= 0) {
